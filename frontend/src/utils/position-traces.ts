...
export function generatePositionTraces(positions: Position[], chartData: DataPoint[]): Trace[] {
  ...
  positions.forEach((position) => {
    ...
    const color = position.side === "long" ? "rgb(59, 130, 246)" : "rgb(239, 68, 68)" // blue for long, red for short
    ...
    traces.push({
      ...
      text: `Entry: ${position.entryPrice}<br>Exit: ${position.exitPrice || "N/A"}<br>Quantity: ${position.quantity}`, //Added hover text
    })
  })
