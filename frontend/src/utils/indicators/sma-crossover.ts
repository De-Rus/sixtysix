...
export class SMACrossoverIndicator extends Indicator {
  ...
  private detectCrossovers(fastSMA: (number | null)[], slowSMA: (number | null)[]): CrossoverPoint[] {
    const crossovers: CrossoverPoint[] = []

    // Start checking from the point where both SMAs have values
    for (let i = Math.max(this.fastPeriod, this.slowPeriod); i < this.data.length; i++) {
      const curr = {
        fast: fastSMA[i],
        slow: slowSMA[i],
      }
  ...
  generateTraces(): Trace[] {
    ...
    if (this.result!.crossovers.length > 0) {
      // Bullish crossovers
      const bullishCrossovers = this.result!.crossovers.filter((c) => c.type === "bullish")
      ...
      // Bearish crossovers
      const bearishCrossovers = this.result!.crossovers.filter((c) => c.type === "bearish")
