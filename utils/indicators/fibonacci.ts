...
export class FibonacciIndicator extends Indicator implements ConfigurableIndicator {
  static readonly name = "Fibonacci Retracement"
  static readonly defaultParams: IndicatorParameter[] = [
    { name: "lookback", type: "number", label: "Lookback Period", value: 100, min: 10, max: 500, step: 10 },
    { name: "lineColor", type: "string", label: "Line Color", value: "rgb(234, 179, 8)" }, // Amber
    { name: "lineOpacity", type: "number", label: "Line Opacity", value: 0.5, min: 0, max: 1, step: 0.1 },
  ...
  constructor(data: DataPoint[], lookback = 100, lineColor = "rgb(234, 179, 8)", lineOpacity = 0.5, showLabels = true) {
    super(data)
  ...
  private findSwingHighLow(): { high: { price: number; time: string }; low: { price: number; time: string } } {
  ...
  calculate(): void {
    ...
    const levels = this.fibLevels.map((fib) => ({
      ...
      color: fib.color,
      label: `${(fib.level * 100).toFixed(1)}% - ${(high.price - priceDiff * fib.level).toFixed(2)}`,
    }))
