  ...
  implements ConfigurableIndicator
{
  ...
  private generateFuturePoints(
    ...
    const slope =
      (endPoint.price - startPoint.price) / (endPoint.index - startPoint.index);
  ...
  private findTrendlines(pivotPoints: TrendPoint[]): Trendline[] {
    ...
    return trendlines.sort(
      (a, b) =>
        new Date(b.endPoint.time).getTime() -
        new Date(a.endPoint.time).getTime()
    );
  }
  ...
  generateTraces(): Trace[] {
    ...
    this.result!.trendlines.forEach((trendline, index) => {
      const color =
        trendline.type === "uptrend" ? this.uptrendColor : this.downtrendColor;
      ...
      traces.push({
        ...
        type: "scatter",
        mode: "lines",
        name: `${trendline.type === "uptrend" ? "Uptrend" : "Downtrend"} ${
          index + 1
        }`,
        line: {
          color,
      ...
      if (index < this.maxFutureLines && trendline.futurePoints.length > 0) {
        traces.push({
          x: [
            ...
            ...trendline.futurePoints.map((p) => p.time),
          ],
          y: [
            trendline.endPoint.price,
            ...trendline.futurePoints.map((p) => p.price),
          ],
          type: "scatter",
          mode: "lines",
          name: `${
            trendline.type === "uptrend" ? "Uptrend" : "Downtrend"
          } Projection ${index + 1}`,
          line: {
    ...
    // Add pivot points
    const highPivots = this.result!.pivotPoints.filter(
      (p) => p.type === "high"
    );
