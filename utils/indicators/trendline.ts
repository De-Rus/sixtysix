...
export class TrendlineIndicator extends Indicator implements ConfigurableIndicator {
  ...
  static readonly defaultParams: IndicatorParameter[] = [
    { name: "lookback", type: "number", label: "Lookback Period", value: 5, min: 1, max: 50, step: 1 },
    { name: "threshold", type: "number", label: "Threshold", value: 0.02, min: 0.001, max: 0.1, step: 0.001 },
    { name: "futurePoints", type: "number", label: "Future Points", value: 20, min: 0, max: 100, step: 1 },
    { name: "maxFutureLines", type: "number", label: "Max Future Lines", value: 5, min: 1, max: 20, step: 1 },
    { name: "uptrendColor", type: "string", label: "Uptrend Color", value: "rgb(59, 130, 246)" },
    { name: "downtrendColor", type: "string", label: "Downtrend Color", value: "rgb(236, 72, 153)" },
  ]
  ...
  constructor(
    ...
    futurePoints = 20,
    maxFutureLines = 5,
    uptrendColor = "rgb(59, 130, 246)",
    downtrendColor = "rgb(236, 72, 153)",
  ) {
    super(data)
    ...
    this.result!.trendlines.forEach((trendline, index) => {
      ...
      if (index < this.maxFutureLines && trendline.futurePoints.length > 0) {
        traces.push({
          ...
          line: {
            color: color + "80", // 80 is for 50% opacity
            width: 2,
            dash: "dot",
          },
          showlegend: false,
    ...
    if (highPivots.length > 0) {
      traces.push({
        ...
        marker: {
          symbol: "circle",
          size: 6,
          color: this.downtrendColor + "80", // 80 is for 50% opacity
        },
        showlegend: false,
      })
    }
    ...
    if (lowPivots.length > 0) {
      traces.push({
        ...
        marker: {
          symbol: "circle",
          size: 6,
          color: this.uptrendColor + "80", // 80 is for 50% opacity
        },
        showlegend: false,
      })
    }
