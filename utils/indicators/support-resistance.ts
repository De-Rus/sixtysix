...
export class SupportResistanceIndicator extends Indicator {
  ...
  private findLocalMaxima(histogram: Map<number, number>): PriceLevel[] {
    ...
    return levels.sort((a, b) => b.strength - a.strength).slice(0, this.maxLevels)
  }
  ...
  private determineLevelType(price: number): "support" | "resistance" {
    ...
    for (let i = Math.max(0, this.data.length - this.lookback); i < this.data.length; i++) {
  ...
  generateTraces(): Trace[] {
    ...
    this.result!.levels.forEach((level) => {
      ...
      traces.push({
        ...
        type: "scatter",
        mode: "lines",
        name: `${level.type === "support" ? "Support" : "Resistance"} ${level.price.toFixed(2)}`,
        line: {
          color,
          width: 1,
          dash: "solid",
