  ...
  implements ConfigurableIndicator
{
  ...
  generateTraces(): Trace[] {
    ...
    this.result!.channels.forEach((channel, index) => {
      ...
      traces.push({
        x: [
          channel.upperLine.start.time,
          channel.upperLine.end.time,
          ...channel.futurePoints.upper.map((p) => p.time),
        ],
        y: [
      ...
      traces.push({
        x: [
          channel.lowerLine.start.time,
          channel.lowerLine.end.time,
          ...channel.futurePoints.lower.map((p) => p.time),
        ],
        y: [
