...
export class ParabolicSARIndicator extends Indicator implements ConfigurableIndicator {
  static readonly name = "Parabolic SAR"
  static readonly defaultParams: IndicatorParameter[] = [
    { name: "initialAF", type: "number", label: "Initial AF", value: 0.02, min: 0.01, max: 0.1, step: 0.01 },
    { name: "maxAF", type: "number", label: "Max AF", value: 0.2, min: 0.1, max: 1, step: 0.1 },
    { name: "increment", type: "number", label: "AF Increment", value: 0.02, min: 0.01, max: 0.1, step: 0.01 },
    { name: "upColor", type: "string", label: "Bullish Color", value: "rgb(34, 197, 94)" }, // Green
    { name: "downColor", type: "string", label: "Bearish Color", value: "rgb(239, 68, 68)" }, // Red
  ]
  ...
  calculate(): void {
    ...
    let sar = isLong ? Math.min(this.data[0].low, this.data[1].low) : Math.max(this.data[0].high, this.data[1].high)
      ...
      if (isLong) {
        if (low < sar) {
          ...
          if (high > extremePoint) {
            extremePoint = high
            accelerationFactor = Math.min(accelerationFactor + this.increment, this.maxAF)
          }
        }
