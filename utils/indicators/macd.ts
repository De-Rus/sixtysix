...
export class MACDIndicator extends Indicator implements ConfigurableIndicator {
  static readonly name = "MACD"
  static readonly config: IndicatorConfig = {
    subplot: true,
    height: 0.3, // 30% of the chart height
  }
  static readonly defaultParams: IndicatorParameter[] = [
    { name: "fastPeriod", type: "number", label: "Fast Period", value: 12, min: 1, max: 50, step: 1 },
    { name: "slowPeriod", type: "number", label: "Slow Period", value: 26, min: 1, max: 100, step: 1 },
    { name: "signalPeriod", type: "number", label: "Signal Period", value: 9, min: 1, max: 50, step: 1 },
    { name: "macdColor", type: "string", label: "MACD Line Color", value: "rgb(59, 130, 246)" }, // Blue
    { name: "signalColor", type: "string", label: "Signal Line Color", value: "rgb(236, 72, 153)" }, // Pink
    { name: "histPositiveColor", type: "string", label: "Histogram Positive Color", value: "rgb(34, 197, 94)" }, // Green
    { name: "histNegativeColor", type: "string", label: "Histogram Negative Color", value: "rgb(239, 68, 68)" }, // Red
  ]
  ...
  setParameters(params: Record<string, any>): void {
    ...
    if (params.histPositiveColor) this.histPositiveColor = params.histPositiveColor
    if (params.histNegativeColor) this.histNegativeColor = params.histNegativeColor
    this.calculate()
  }
  ...
  validateParameters(params: Record<string, any>): void {
    // Basic validation
    if (params.fastPeriod && (params.fastPeriod < 1 || params.fastPeriod > 50)) {
      throw new Error("Fast Period must be between 1 and 50")
    }
    if (params.slowPeriod && (params.slowPeriod < 1 || params.slowPeriod > 100)) {
      throw new Error("Slow Period must be between 1 and 100")
    }
    if (params.signalPeriod && (params.signalPeriod < 1 || params.signalPeriod > 50)) {
      throw new Error("Signal Period must be between 1 and 50")
    }
  }

  getParameter(name: string): IndicatorParameter | undefined {
    return MACDIndicator.defaultParams.find((param) => param.name === name)
  }

  setParameter(param: IndicatorParameter): void {
  ...
  removeParameter(name: string): void {
    // Cannot remove parameters from MACD
  }

  addParameters(params: IndicatorParameter[]): void {
    // Cannot add parameters to MACD
  }

  hasParameter(name: string): boolean {
    return MACDIndicator.defaultParams.some((param) => param.name === name)
  }
  ...
  calculate(): void {
    ...
    for (let i = startIndex; i < prices.length; i++) {
      macd.push(fastEMA[i - startIndex + (this.slowPeriod - this.fastPeriod)] - slowEMA[i - startIndex])
    }
