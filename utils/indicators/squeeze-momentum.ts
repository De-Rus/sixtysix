...
export class SqueezeMomentumIndicator extends Indicator implements ConfigurableIndicator {
  static readonly name = "Squeeze Momentum"
  static readonly config: IndicatorConfig = {
    subplot: true,
    height: 0.3,
  }
  static readonly defaultParams: IndicatorParameter[] = [
    { name: "bbPeriod", type: "number", label: "BB Period", value: 20, min: 1, max: 50, step: 1 },
    { name: "bbMultiplier", type: "number", label: "BB Multiplier", value: 2, min: 0.1, max: 5, step: 0.1 },
    { name: "kcPeriod", type: "number", label: "KC Period", value: 20, min: 1, max: 50, step: 1 },
    { name: "kcMultiplier", type: "number", label: "KC Multiplier", value: 1.5, min: 0.1, max: 5, step: 0.1 },
    { name: "momentumPeriod", type: "number", label: "Momentum Period", value: 12, min: 1, max: 50, step: 1 },
    { name: "sma20Period", type: "number", label: "Fast MA Period", value: 20, min: 1, max: 50, step: 1 },
    { name: "sma50Period", type: "number", label: "Slow MA Period", value: 50, min: 1, max: 100, step: 1 },
    { name: "squeezeColor", type: "string", label: "Squeeze Color", value: "rgb(234, 179, 8)" }, // Amber
    { name: "upColor", type: "string", label: "Up Color", value: "rgb(34, 197, 94)" }, // Green
    { name: "downColor", type: "string", label: "Down Color", value: "rgb(239, 68, 68)" }, // Red
    { name: "sma20Color", type: "string", label: "Fast MA Color", value: "rgb(59, 130, 246)" }, // Blue
    { name: "sma50Color", type: "string", label: "Slow MA Color", value: "rgb(147, 51, 234)" }, // Purple
  ]
  ...
  private calculateStdDev(values: number[], sma: number[], period: number): number[] {
    const stdDev: number[] = []
    for (let i = 0; i < values.length; i++) {
      if (i < period - 1) {
        stdDev.push(0)
  ...
  calculate(): void {
    ...
    const bbUpper = bbSMA.map((sma, i) => sma + this.bbMultiplier * bbStdDev[i])
    const bbLower = bbSMA.map((sma, i) => sma - this.bbMultiplier * bbStdDev[i])
  ...
  generateTraces(): Trace[] {
    ...
    return [
      ...
      // Positive momentum bars
      {
        ...
        marker: {
          color: this.result!.squeeze.map((sq) => (sq ? this.squeezeColor : this.upColor)),
        },
        yaxis: "y2",
        showlegend: false,
      },
      // Negative momentum bars
      {
        ...
        name: "Squeeze Momentum -",
        marker: {
          color: this.result!.squeeze.map((sq) => (sq ? this.squeezeColor : this.downColor)),
        },
        yaxis: "y2",
        showlegend: false,
      },
