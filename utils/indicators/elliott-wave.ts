...
export class ElliottWaveIndicator extends Indicator implements ConfigurableIndicator {
  static readonly name = "Elliott Wave"
  static readonly defaultParams: IndicatorParameter[] = [
    { name: "lookback", type: "number", label: "Lookback Period", value: 10, min: 5, max: 50, step: 1 },
    { name: "sensitivity", type: "number", label: "Wave Sensitivity", value: 0.02, min: 0.001, max: 0.1, step: 0.001 },
    { name: "impulseColor", type: "string", label: "Impulse Wave Color", value: "rgb(34, 197, 94)" }, // Green
    { name: "correctiveColor", type: "string", label: "Corrective Wave Color", value: "rgb(239, 68, 68)" }, // Red
    { name: "showLabels", type: "boolean", label: "Show Labels", value: true },
    { name: "labelSize", type: "number", label: "Label Size", value: 12, min: 8, max: 24, step: 1 },
  ]
    ...
    if (this.showLabels) {
      waves.forEach((wave) => {
        traces.push({
          ...
          textfont: {
            size: this.labelSize,
            color: wave.type === "impulse" ? this.impulseColor : this.correctiveColor,
          },
          hoverinfo: "none",
          showlegend: false,
        })
