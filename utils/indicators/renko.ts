...
export class RenkoIndicator extends Indicator implements ConfigurableIndicator {
  static readonly name = "Renko"
  static readonly config = { subplot: false }
  static readonly defaultParams: IndicatorParameter[] = [
    { name: "blockSize", type: "number", label: "Block Size", value: 0.5, min: 0.1, max: 10, step: 0.1 },
    { name: "upColor", type: "string", label: "Bullish Color", value: "rgba(34, 197, 94, 0.2)" },
    { name: "downColor", type: "string", label: "Bearish Color", value: "rgba(239, 68, 68, 0.2)" },
    { name: "lineWidth", type: "number", label: "Line Width", value: 2, min: 1, max: 5, step: 1 },
  ]
  ...
  generateTraces(): Trace[] {
    ...
    segments.forEach((segment, index) => {
      const segmentTimes = top.slice(segment.start, segment.end + 1).map((t) => t.time)
      const segmentTop = top.slice(segment.start, segment.end + 1).map((t) => t.value)
      const segmentBottom = bottom.slice(segment.start, segment.end + 1).map((t) => t.value)
      ...
      traces.push({
        ...
        type: "scatter",
        mode: "none",
        name: `Renko ${segment.trend === "up" ? "Bullish" : "Bearish"} ${index + 1}`,
        showlegend: false,
        hoverinfo: "none",
      })
    })
